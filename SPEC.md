...
3. 群聊只接受 `chat_type = group` 且明确 `@` 当前飞书机器人本身
...
11. 解析最终输出中的 `[[wiki-write:https://...]] ... [[/wiki-write]]` 块
12. 如存在 wiki-write 块，按块内 Markdown 覆盖写入目标 Wiki / Docx 页面
13. 先回复文本，再按顺序发送图片
14. 删除第 6 步添加的 ack reaction
15. 记录消息、执行、会话、授权与配对状态到 SQLite
...
- 群内仅 `@` 当前机器人触发执行，普通文本忽略
- `@` 触发时，临时从飞书拉取该群最近 N 条消息作为上下文
- 懒抓取：不实时监测群消息，不做本地缓存，`@` 触发时才拉取
